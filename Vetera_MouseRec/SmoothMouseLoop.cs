...
namespace Vetera_MouseRec
{
    class SmoothMouseLoop
    {
        ...
        public void Smooth(int x, int y)
        {
            ...
            Cash.smooth_mouse_time = GetTime(x, y);

            Play.SmoothText = Cash.smooth_mouse_time.ToString();
            ...
            do
            {
                ...
                timeout = (ms > (Cash.smooth_mouse_time + 10));
                completed = (x == Cursor.Position.X || y == Cursor.Position.Y);
                stop = Cash.STOP_PLAYBACK;
        ...
        private int GetTime(int x, int y)
        {
            ...
            return Remap(distance, 0, distance_max, Cash.smooth_mouse_time_min, Cash.smooth_mouse_time_max);
        }

        private float GetSpeed(int start, int end)      //return pixel/ms
        {
            int distance = start - end;
            return ((float)distance / (float)Cash.smooth_mouse_time);

        }

        private int GetNewRoundPos(float speed, int ms)     // return pixel
        {
            return (int)(speed * (Cash.smooth_mouse_time - ms));
        }
