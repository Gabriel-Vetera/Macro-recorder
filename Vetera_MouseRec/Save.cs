...
namespace Vetera_MouseRec
{
    class Save
    {
        ...
        static void InsertDataCollection(DataCollection collection, int IDCollection)
        {
            Insert_I_Data(collection, IDCollection);

            for (int i = 0; i < collection.Data.Count; i++)
            {
                InsertData(collection.Data[i], IDCollection, i, collection.Order[i]);
            }
        ...
        static void InsertData(Data data, int IDCollection, int IdList, int PlayOrder)
        {
            Insert_L_Data(data.Name, IDCollection, IdList, PlayOrder);
            Insert_M_Data(data.MouseData, IDCollection, IdList);
            Insert_K_Data(data.KeyboardData, IDCollection, IdList);

        }

        static void Insert_L_Data(String Name, int IDCollection, int IdList, int PlayOrder)
        {
            sqlite_cmd.CommandText = "INSERT INTO DataList(Name , PlayOrder , ID, CollectionID) VALUES('" + Name + "', " + PlayOrder + ", " + IdList + ", " + IDCollection + "); ";
            sqlite_cmd.ExecuteNonQuery();
        }

        static void Insert_M_Data(MouseData mouse, int IDCollection, int IdList)
        {
            int[] x = mouse.getX();
            int[] y = mouse.getY();
            int[] m = mouse.getMouseClick();
            long[] delay = mouse.getDelay();

            int size = x.Length;

            for (int i = 0; i < size; i++)
            {
                sqlite_cmd.CommandText = "INSERT INTO DataMouse(X, Y, M, Delay, ItemID, ListID, CollectionID) VALUES(" + x[i] + ", " + y[i] + ", " + m[i] + ", '" + delay[i] + "', " + i + ", " + IdList + ", " + IDCollection + "); ";
                sqlite_cmd.ExecuteNonQuery();
            }
        ...
        static void Insert_K_Data(KeyboardData keyboard, int IDCollection, int IdList)
        {

            keyboard.finish = true;
            List<KeyData> key_list = keyboard.getKeyboarData();

            for (int i = 0; i < key_list.Count; i++)
            {
                sqlite_cmd.CommandText = "INSERT INTO DataKeyboard(KeysINT, Start, Stop ,ItemID, ListID, CollectionID) VALUES(" + (int)key_list[i].getKey() + "," + key_list[i].getStart() + ", " + key_list[i].getStop() + ", " + i + ", " + IdList + ", " + IDCollection + "); ";
                sqlite_cmd.ExecuteNonQuery();
        ...
        static void Insert_L_DataNEW(List<String> Name, int IDCollection, List<int> PlayOrder)
        {
            ...
            //OutPut = OutPut + Get_L_DataText(Name, PlayOrder, IDCollection);
        ...
        static String[] Get_K_DataText(List<KeyData> key_list, int ListID, int IDCollection)
        {
            List<String> OutPut = new List<String>();
        ...
        static String[] Get_L_DataText(List<String> Name, List<int> PlayOrder, int IDCollection)
        {
            ...
            List<String> OutPut = new List<String>();
            // OutPut.Add("('" + Name[0] + "', " + PlayOrder[0] + ", " + 0 + ", " + IDCollection + ")");
        ...
        public void Start()
        {
            ...
            // Form1.Invoke(() => textBox1.Text += "hi");
            //Form1.AppendTextBox("Save and compress..");
            ...
            for (int i = 0; i < dataCollections.Count; i++)
            {
                ...
                //InsertDataCollection(dataCollections[i], i);
            }
