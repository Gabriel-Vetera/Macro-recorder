...
namespace Vetera_MouseRec
{
    public class SimulateMouse
    {
        ...
        private void runRecordMouse()
        {
            ...
            while (count < max && !Cash.STOP_PLAYBACK)
            {
                if (!Cash.isPause())
                {
                    ...
                    if (Cash.isKeyboardReadyForPause() && !Cash.wait_for_paus_m && Cash.paus_request)
                    {
                ...
                {
                    waited_delay = time;
                    Cash.x_mouse = x_arr[count - 1];
                    Cash.y_mouse = y_arr[count - 1];
                    start = DateTime.Now;
                }
                Thread.Sleep(1);

            }
            Cash.playback_finish = true;
        ...
        private void performMouseEvent(int pos)
        {
            ...
            {
                case 0:
                    Cash.wait_for_paus_m = false;
                    SetCursorPos(x, y); //move
                    break;

                case 1:
                    Cash.wait_for_paus_m = true;
