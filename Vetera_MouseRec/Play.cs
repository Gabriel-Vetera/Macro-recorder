...
namespace Vetera_MouseRec
{
    public partial class Play : Form
    {
        ...
        {
            set { update_text = Cash.infoSmooth + value + " ms"; }
        }
        ...
        private void SetColor()
        {
            ...
            play_text_name.BackColor = Cash.color_item_backgrund[Cash.theame];
            play_text_name.TextColor = Cash.color_text[Cash.theame];

            play_text_title.BackColor = Cash.color_item_backgrund[Cash.theame];
            play_text_title.TextColor = Cash.color_text[Cash.theame];

            ProgressBar.ProgressBackgrund = Cash.color_item_backgrund[Cash.theame];
            ProgressBar.ProgressForeground = Cash.color_contrast[Cash.theame];
            ProgressBar.TextColor = Cash.color_text[Cash.theame];
        }
        ...
        private void update_Tick(object sender, EventArgs e)
        {
            ...
            if (!SmoothMode)
            {
                ...
                else
                {
                    ...
                    String text = Cash.infoPause + count_now.ToString() + "/" + _items.ToString();
                ...
                {
                    ProgressBar.Value = 0;
                    String text = Cash.infoNextLoopStartIn + ((int)(Countdown / 10)).ToString();
                    ProgressBar.text = text;
                }
            }
        }
        ...
        private void update_progress()
        {
            ...
            if (!Cash.playback_finish)
            {
                ...
                if (Cash.paus_request && !Cash.isPause())
                {
                    long count_now = KeyboardItems + MouseItems;
                    ProgressBar.text = Cash.infoWaitForPause + "\n" + count_now.ToString() + "/" + _items.ToString();
                    ProgressBar.Value = Remap(time_passed, 0, _time, 0, 10000);
                }
